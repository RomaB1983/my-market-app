...
class OrderServiceTest {
    ...
    void test_createOrder_Success() {
        ...
        when(cartService.removeAllItems(eq(USER_ID)))
                .thenReturn(Mono.empty());
        when(cartService.getCartItems(eq(USER_ID)))
       ...
       when(paymentService.createPayment(eq(USER_ID), anyLong()))
               .thenReturn(Mono.just(PaymentStatus.SUCCESS));

        Mono<OrderDto> result = orderService.createOrder(USER_ID);
        ...
        verify(cartService, times(1)).removeAllItems(eq(USER_ID));
    }

    @Test
    void test_createOrder_getTotalSum_isOk() {
        // Подготовка данных
        List<ItemDto> cartItems = List.of(itemDto1, itemDto2);

        Long result = orderService.getTotalSum(cartItems);
    ...
    void test_getAllOrders_Success() {
        when(orderRepository.findByUsername(eq(USER_ID)))
        ...
        Mono<List<OrderDto>> result = orderService.getAllOrders(USER_ID);
        ...
        verify(orderRepository, times(1)).findByUsername(USER_ID);
    ...
    void test_getOrderById_Success() {
        when(orderRepository.findByIdAndUsername(eq(1L), eq(USER_ID)))
                .thenReturn(Mono.just(order));
        ...
        Mono<OrderDto> result = orderService.getOrderById(USER_ID, 1L);
        ...
        verify(orderRepository, times(1)).findByIdAndUsername(1L, USER_ID);
        verify(itemService, times(2)).getItemByIdForOrder(anyLong());
    }

    @Test
    void test_getOrderById_NotFound() {
        when(orderRepository.findByIdAndUsername(eq(999L), eq(USER_ID)))
                .thenReturn(Mono.empty());

        Mono<OrderDto> result = orderService.getOrderById(USER_ID, 999L);

        StepVerifier.create(result)
                .expectNextCount(0)
                .verifyComplete();

        verify(orderRepository, times(1)).findByIdAndUsername(999L, USER_ID);
    ...
    void test_createOrder_RepositorySaveError() {
        ...
        verify(cartService, never()).removeAllItems(anyString());
    }
